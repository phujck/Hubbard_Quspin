...
outfile = './Data/Uzero_expectations:{}sites-{}up-{}down-{}t0-{}U-{}cycles-{}steps-{}pbc.npz'.format(L, N_up, N_down, t0, U2,
                                                                                               cycles,
                                                                                               n_steps, pbc)
# U2=1.0001*t0
outfile2 = './Data/mimic_expectations:{}sites-{}up-{}down-{}t0-{}U-{}cycles-{}steps-{}pbc.npz'.format(L, N_up, N_down, t0, U2,
                                                                                               cycles,
                                                                                               n_steps, pbc)
...
J_field2 = expectations2['mimic_current']
E=expectations['H']
# E2=expectations2['H']
R=np.abs(expectations['neighbour'])
theta=np.angle(expectations['neighbour'])
R2=np.abs(expectations2['mimic_neighbour'])
theta2=np.angle(expectations2['mimic_neighbour'])
phi=expectations2['mimic_phi']

def unwrap(phi):
    phi=phi-phi[0]
    for j in range(len(phi)-1):
        if phi[j+1]-phi[j] >1.9*np.pi:
            phi[j+1:]-=2*np.pi
        elif phi[j+1]-phi[j] < -1.9*np.pi:
            phi[j+1:]+=2*np.pi
    return phi

def unwrap_nozero(phi):
    for j in range(len(phi)-1):
        if phi[j+1]-phi[j] >1.9*np.pi:
            phi[j+1:]-=2*np.pi
        elif phi[j+1]-phi[j] < -1.9*np.pi:
    ...
    return phi
theta=unwrap(theta)
theta2=unwrap_nozero(theta2)

plt.plot(times,R,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U/t0))
plt.plot(times,R2,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
plt.legend()
plt.xlabel('Time (cycles)')
plt.ylabel('$R(\\psi)$')
plt.show()

plt.plot(times,theta,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U/t0))
plt.plot(times,theta2,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
plt.legend()
plt.xlabel('Time (cycles)')
plt.ylabel('$\\theta(\\psi)$')
plt.show()

# plt.plot(times,R*np.sin(theta),label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U/t0))
# plt.plot(times,R*np.sin(theta2),label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
# plt.legend()
# plt.xlabel('Time (cycles)')
# plt.ylabel('$R(\\psi)*\\sin(\\theta(\\psi))$')
# plt.show()
#
# plt.plot(times,R*np.cos(theta),label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U/t0))
# plt.plot(times,R*np.cos(theta2),label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
# plt.legend()
# plt.xlabel('Time (cycles)')
# plt.ylabel('$R(\\psi)*\\cos(\\theta(\\psi))$')
# plt.show()
...
plt.xlabel('Time(cycles)')
plt.ylabel('$J(t)$')
plt.plot(times,J_field,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U/t0))
plt.plot(times,J_field2,linestyle='dashed',label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
plt.legend()
plt.show()


for U2 in [0.1*t0,6*t0,10*t0,20*t0]:
    outfile2 = './Data/mimic_expectations:{}sites-{}up-{}down-{}t0-{}U-{}cycles-{}steps-{}pbc.npz'.format(L, N_up,
                                                                                                          N_down, t0,
                                                                                                          U2,
                                                                                                          cycles,
                                                                                                          n_steps, pbc)
    ...
    """show the expectations available here"""
    print('expectations available: {}'.format(expectations2.files))
    sites = [j for j in range(L)]
    sites2 = sites
    """example for using the analysis code"""
    # J_field = expectations['current']
    J_field2 = expectations2['mimic_current']
    phi=expectations2['mimic_phi']
    plt.plot(phi,J_field2,label='$\\frac{U}{t_0}=$'+ '{:.1f}'.format(U2/t0))
plt.xlabel('$\\Phi(t)$')
plt.ylabel('$J(t)$')
plt.legend()
plt.show()
