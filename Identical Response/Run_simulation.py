...
if mimic_phi:
    loadfile = './Data/mimic_expectations:{}sites-{}up-{}down-{}t0-{}U-{}cycles-{}steps-{}pbc.npz'.format(L, N_up, N_down,
                                                                                                         t0, U2, cycles,
                                                                                                         n_steps, pbc)
    expectations = dict(np.load(loadfile))
    """set up parameters for saving expectations later"""
    outfile = './Data/Uzero_expectations:{}sites-{}up-{}down-{}t0-{}U-{}cycles-{}steps-{}pbc.npz'.format(L, N_up, N_down,
                                                                                                   t0, U2, cycles,
                                                                                                   n_steps, pbc)
    print(expectations.keys())

    phi=interp1d(times, expectations['mimic_phi'], fill_value='extrapolate', bounds_error=False, kind='cubic')
else:
...
# print(psi_0.shape)
# E, psi_0 = ham.eigsh(k=2, which='SA')
# E=E[1]
# psi_0=psi_0[:,1]+psi_0[:,0]
...
print("ground state calculated, energy is {:.2f}".format(E))
# psi_0.reshape((-1,))
# psi_0=psi_0.flatten
